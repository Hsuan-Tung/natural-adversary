...
from models import Seq2Seq, MLP_D, MLP_G, MLP_I, MLP_I_AE, JSDistance, Baseline_Embeddings, Baseline_LSTM, Seq2SeqCAE
...
def evaluate_inverter(data_source, epoch,
    ...
    for batch in data_source:
        source, target, lengths = batch
        with torch.no_grad():
        	source = to_gpu(args.cuda, source)
        	target = to_gpu(args.cuda, target)
...
if __name__ == '__main__':
    ...
    one = to_gpu(args.cuda, torch.tensor(1, dtype=torch.float))#torch.FloatTensor([1]))
    mone = one * -1
