...
class SNLIDataset(data.Dataset):
    ...
    def __init__(self, path="./data/classifier", train=True,
        ...
        if os.path.exists(self.root+"/sent_ids.pkl"):
            with open(self.root+"/sent_ids.pkl", 'rb') as f:
            	self.sentence_ids = pkl.load(f, encoding="bytes")
            # self.sentence_ids = pkl.load(open(self.root+"/sent_ids.pkl",'r'), encoding="bytes")
            # print(self.sentence_ids, type(self.sentence_ids))
            self.sentence_ids = { key.decode(): val.decode() for key, val in self.sentence_ids.items() }
        else:
